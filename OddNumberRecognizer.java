...
public class OddNumberRecognizer {
	/**
	...
	private static String executeFSM(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				if (currentChar == '2' || currentChar == '4' || currentChar == '6' ||
						currentChar == '8') {
					state = 1;
					break;
				}
			...
			case 1: 
				...
				else if (currentChar == '0' || currentChar == '2' || currentChar == '4' ||
			...
			case 2: 
				...
				else if (currentChar == '2' || currentChar == '4' ||
						currentChar == '6' || currentChar == '8'
						) {
					state = 1;
					break;
		...
		case 1:
			// State 1 is not final state, so we can display a very specific error message.
			return "This character must be a 1, 3, 5, 7, or 9 the the value to be odd.\n" + 
					displayInput(input, currentCharNdx);
		...
		case 3:
			// States 3 is a final state, so we must be sure there are no more characters.
			if (currentCharNdx<=input.length())
				return "This character must be 1, 3, 5, 7, or 9 the the value to be odd.\n" +
					displayInput(input, currentCharNdx);
